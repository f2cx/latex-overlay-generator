...
angular.module('latexOverlayApp')
        .controller('SelectLocalCtrl', function ($scope, $filter) {
            ...
            $scope.statuses = [
                {value: 'annotatedFigureBox', text: 'LaTeX Annotation Box'},
                {value: 'annotatedFigureText', text: 'LaTeX Text Overlay'}
            ];

            $scope.showStatus = function () {
                var selected = $filter('filter')($scope.statuses, {value: $scope.selectedItem.type});
                return ($scope.selectedItem.type && selected.length) ? selected[0].text : 'Not set';
            };
        }).
        ...
        .controller('MainCtrl', function ($scope, $rootScope, ngDialog, $http) {
            ...
            function getLatexCode() {
                ...
                for (var i = 0; i < $rootScope.overlay.items.length; i++) {
                    ...
                    if (item.type === 'annotatedFigureText') {

                        var w = Number((parseFloat(item.x1) - parseFloat(item.x0)).toFixed(4));

                        rtn = rtn + '\t\\' + item.type + '{' + item.x0 + ',' + item.y0 + '}{white}{' + w + '}{' + item.label + '}\n';

                    } else {
                        rtn = rtn + '\t\\' + item.type + '{' + item.x0 + ',' + item.y0 + '}{' + item.x1 + ',' + item.y1 + '}{' + item.label + '}{' + item.x2 + ',' + item.y2 + '}%' + item.position + '\n';
                    }
            ...
            $scope.parseLatexCode = function () {
                ...
                while ((tag = myRe.exec(str)) !== null) {
                    ...
                    var itemA = {
                        type: 'annotatedFigureText',
                        label: tag[5],
                        x0: tag[1],
                        y0: tag[2],
                        x1: (parseFloat(tag[1]) + parseFloat(tag[4])),
                        y1: (parseFloat(tag[2]) + 0.1),
                        x2: tag[1],
                        y2: tag[2],
                        position: 'na'
                    };

                    $rootScope.overlay.items.push(itemA);
                }

                //\annotatedFigureText{0.085,0.49}{white}{0.1695}{A}
            ...
            function addOverlay(item) {
                ...
                if (item.type === 'annotatedFigureText') {
                    ...
                    var labelPosition = new fabric.Point(dec2x(item.x0), dec2y(item.y0));

                    group.setPositionByOrigin(labelPosition, 'left', 'bottom');

                } else {

                    var textA = new fabric.Text(item.label, {
                        fontSize: 10,
                        originX: 'center',
                        originY: 'center'
                    });
                    group = new fabric.Group([circle, textA], {
                        left: 0,
                        top: 0,
                        selectable: false
                    });

                    var labelPositionA = new fabric.Point(dec2x(item.x2), dec2y(item.y2));

                    group.setPositionByOrigin(labelPositionA, 'center', 'center');
